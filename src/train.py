...
from nn.nn_options_torch import optimizers, loss_functions, activation_functions
...
def rl_algorithm(actor: Actor, state_manager: StateManager, mcts_state_manager: StateManager, display: GameBoardDisplay):
    logging.basicConfig(
    ...
    for g_a in tqdm(range(config.NUM_EPISODES + 1)):
        ...
        while not state_manager.check_winning_state():
            ...
            board_state = convert_board_state_to_tensor(
                mcts_tree.root.state, mcts_tree.root.player_to_move
        ...
        train_dataset = TensorDataset(torch.tensor(X).float(), torch.tensor(y_actor).float(), torch.tensor(y_critic).float())
