...
class HexBoardDisplay(GameBoardDisplay):
    ...
    def display_board(
        ...
        if not self.init:
            self.fig = plt.figure(
                figsize=(
                    self.width * self.board_skewness_factor * PX,
                    self.height * PX,
                ),
                num="Hex",
            )
        ...
        horizontal_offset_factor = (self.width / len(board) - 1) * (
            self.board_skewness_factor
        )
        ...
        for i in range(len(board)):
            ...
            posX = self.width / 2 - (horizontal_offset_factor * (len(board[i]) - 1))
            ...
            for j in range(len(board[i])):
                ...
                if j < len(board[i]) - 1:
                    ...
                    posY_right = posY
                    self.ax.plot(
                        [posX, posX_right], [posY, posY_right], "-", color="black"
                    )
                ...
                if i < len(board) - 1 and j < len(board[i + 1]):
                    ...
                    self.ax.plot(
                        [posX, posX_bottom_right],
                        [posY, posY_bottom_right],
                        "-",
                        color="black",
                    )
                ...
                if i < len(board) - 1 and (j > 0 or len(board[i]) < len(board[i + 1])):
                    ...
                    self.ax.plot(
                        [posX, posX_bottom_left],
                        [posY, posY_bottom_left],
                        "-",
                        color="black",
                    )
    ...
    def _convert_to_display_shape(self, board):
        ...
        board = np.array(
            [
                [PositionPlayer((i, j), board[i][j]) for j in range(len(board))]
                for i in range(len(board))
            ]
        )
